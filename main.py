...
async def start(client: Client, msg: Message):
    ...
    await start_message.edit_text(
        Data.START.format(msg.from_user.mention) +
        "Checking status Ok... **Bot Made BY @fr_sammm11👨🏻‍💻**🔍\n\n"
        "Progress:[🟩🟩🟩🟩🟩🟩🟩🟩🟩] 100%\n\n"
        "**Available Commands:**\n"
        "/upload - Upload TXT files\n"
        "/king - Alternative for Upload\n"
        "/advance - Advance download mode\n"
        "/spidy - Special handler\n"
...
async def process_txt_file(bot: Client, m: Message, command_name: str):
    await m.delete()
    ...
    # Conditional Token Input Logic
    MR = token
    if command_name in ["king", "upload", "advance"]:
        await editable.edit("**Enter Your PW Token For 𝐌𝐏𝐃 𝐔𝐑𝐋  or send 'Not' for use default**")
        input4: Message = await bot.listen(editable.chat.id)
        raw_text4 = input4.text
        await input4.delete(True)
        if raw_text4.lower() not in ['not', 'unknown']:
            MR = raw_text4
    # Commands like spidy, alpha, bravo typically don't ask for token or use default
    ...
    try:
        for i in range(arg-1, len(links)):
            ...
            # ... (Site specific URL processing logic preserved) ...
            if "visionias" in url:
            ...
            if 'amazonaws.com' in url:
                url =  f"https://master-api-v3.vercel.app/adda-mp4-m3u8?url={url}&quality={raw_text2}&token={MR}"
            ...
            if 'sec-prod-mediacdn.pw.live' in url:
             ...
             url = f"https://pwplayer-0e2dbbdc0989.herokuapp.com/player?url=https://d1d34p8vz63oiq.cloudfront.net/{vid_id}/master.mpd?token={MR}"
            ...
            if "drive" in url:
                ...
                try:
                    ...
                    await m.reply_text(f"PDF processing failed: {str(e)}")
                    continue
                finally:
            ...
            # Improved Image Download Logic
                ...
                try:
                    ...
                    filename = f"{name1}{ext}"
                    # Try using requests first for direct images
                    try:
                    ...
                    await bot.send_document(chat_id=m.chat.id, document=filename, caption=cc3)
                    count += 1
                    ...
                    await m.reply_text(f"Failed to download image: {str(e)}")
                    continue
...
@bot.on_message(filters.command(["king","upload", "advance", "spidy", "alpha", "bravo"]) )
async def txt_handler(bot: Client, m: Message):
    command_name = m.command[0]
    await process_txt_file(bot, m, command_name)
